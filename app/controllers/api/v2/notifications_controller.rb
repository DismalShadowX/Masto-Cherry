...
class Api::V2::NotificationsController < Api::BaseController
  ...
  def index
    ...
    render json: @notifications, each_serializer: REST::NotificationGroupSerializer, relationships: @relationships
  end

  def show
    @notification = current_account.notifications.without_suspended.find_by!(group_key: params[:id])
    render json: @notification, serializer: REST::NotificationGroupSerializer
  end
  ...
  def load_notifications
    ...
    # TODO: load group information (count, most recent ID, accounts sample)
